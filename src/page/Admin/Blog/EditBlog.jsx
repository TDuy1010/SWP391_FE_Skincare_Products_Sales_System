...
const EditBlog = ({ visible, onCancel, onSuccess, blogData }) => {
  ...
  useEffect(() => {
    if (blogData && blogData.contents) {
      const initialContents = {
        content_0: blogData.contents[0] || "",
        ...blogData.contents.slice(1).reduce((acc, content, index) => {
          acc[`content_${index + 1}`] = content;
          return acc;
        }, {}),
      };
      form.setFieldsValue({
      ...
      setExtraFields(blogData.contents.slice(1).map((_, index) => ({ key: index + 1 })));
      setThumbnails(blogData.thumbnails || []);
    }
  }, [blogData]);
  ...
  const onFinish = async (values) => {
    if (values.title && values.content_0 && values.author && values.thumbnail?.length > 0) {
      try {
        ...
        const thumbnailsArray = [file, ...extraFields.map((_, index) => values[`images_${index + 1}`]?.[0]?.originFileObj || "")];
